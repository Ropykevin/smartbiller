...
def send_custom_invoice_email(tenant, invoice_buffer, month, year, total_amount, additional_charges=None):
    ...
    # Email configuration (would need to be set up)
    smtp_server = "smtp.gmail.com"
    smtp_port = 587
    sender_email = "noreply@smartbiller.co.ke"
    ...
    msg['To'] = tenant.email
    msg['Subject'] = f"Rent Invoice - {month} {year}"
    ...
    part.set_payload(invoice_buffer.read())
    encoders.encode_base64(part)
    part.add_header(
        'Content-Disposition',
        f'attachment; filename= "rent_invoice_{month}_{year}.pdf"'
    )
    msg.attach(part)
    ...
    try:
        ...
        text = msg.as_string()
        server.sendmail(sender_email, tenant.email, text)
        server.quit()
