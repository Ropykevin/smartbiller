...
from .models import Landlord, Property, Unit, Tenant, RentLog, PropertyImage, UnitImage, RentLogImage, ExitNotice, Invoice, generate_receipt_number, generate_invoice_number
...
def log_rent(tenant_id):
    ...
    if request.method == 'POST':
        ...
        if existing_payment:
            ...
            existing_payment.date_paid = datetime.strptime(date_paid, '%Y-%m-%d')  # Update to latest payment date
            
            # Recalculate status based on total amount paid for this month
            total_paid_for_month = existing_payment.amount_paid
            if total_paid_for_month >= monthly_rent:
                existing_payment.status = "Paid"
            elif total_paid_for_month > 0:
                existing_payment.status = "Partial"
            else:
            ...
            if remaining_balance <= 0:
                flash(f'Installment payment logged successfully! Total paid for {month_paid_for}: KES {total_paid_for_month:,.2f}. Status: {existing_payment.status}.')
            else:
...
def upgrade():
    ...
    try:
        ...
        if not current_plan:
            ...
            flash('No active subscription found. Please contact support.', 'error')
            return redirect(url_for('main.dashboard'))
